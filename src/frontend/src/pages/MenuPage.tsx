...
const pizzaItems: SizedItem[] = [
  ...
  },
  {
    ...
    sizes: [
      { label: "Regular", price: 100 },
      { label: "Medium", price: 155 },
      { label: "Large", price: 255 },
    ],
  },
  {
...
function SizedItemCard({
  item,
  category,
}: { item: SizedItem; category: string }) {
        ...
        <div className="flex flex-wrap gap-1.5 mt-2">
          {item.sizes.map((s) => (
            <button
              key={s.label}
              type="button"
              onClick={() => setSelectedSize(item.sizes.indexOf(s))}
              className={`px-2.5 py-1 rounded-full text-xs font-medium border transition-all ${
                item.sizes[selectedSize].label === s.label
...
function FixedItemCard({
  item,
  category,
}: { item: FixedItem; category: string }) {
  const { addToCart } = useCart();
  const [added, setAdded] = useState(false);
  ...
  return (
    <div className="pizza-card p-4 flex items-center justify-between gap-3">
      ...
      <div className="flex items-center gap-3 flex-shrink-0">
        {item.price ? (
          <span className="font-bold text-base" style={{ color: "#B71C1C" }}>
            ₹{item.price}
          </span>
        ) : (
    ...
    <div className="min-h-screen" style={{ backgroundColor: "#FAFAFA" }}>
      ...
      <div className="red-gradient-bg py-10 px-4 sm:px-6 text-center">
        ...
        <p style={{ color: "#FFCDD2" }} className="text-base">
          Fresh, delicious food made with love — Sonauli Road Pipiganj,
          Gorakhpur
        </p>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10 space-y-12">
        <section>
          <SectionHeader
            emoji="🍕"
            title="Pizza"
            subtitle="Choose your size: Regular / Medium / Large"
          />
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {pizzaItems.map((item) => (
              <SizedItemCard key={item.name} item={item} category="Pizza" />
            ))}
          </div>
        </section>
        ...
        <section>
          <SectionHeader
            emoji="🍜"
            title="Chinese"
            subtitle="Choose your portion: Half / Full"
          />
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {chineseItems.map((item) => (
              <SizedItemCard key={item.name} item={item} category="Chinese" />
            ))}
          </div>
        </section>
        ...
        <section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {riceItems.map((item) => (
              <SizedItemCard key={item.name} item={item} category="Rice" />
            ))}
          </div>
        </section>
        ...
        <section>
          <SectionHeader
            emoji="🍔"
            title="Burger"
            subtitle="Freshly made veg & classic burgers"
          />
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {burgerItems.map((item) => (
              <FixedItemCard key={item.name} item={item} category="Burger" />
            ))}
          </div>
        </section>
        ...
        <section>
          <SectionHeader
            emoji="🍟"
            title="Snacks & Extras"
            subtitle="Perfect sides and add-ons"
          />
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {snackItems.map((item) => (
              <FixedItemCard key={item.name} item={item} category="Snacks" />
            ))}
          </div>
        </section>
      </div>

...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50" style={{ backgroundColor: '#B71C1C', boxShadow: '0 4px 20px rgba(183,28,28,0.3)' }}>
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <Link to="/" className="flex items-center gap-2 group">
            ...
            <div className="flex flex-col leading-tight">
              <span className="text-white font-bold text-lg leading-none" style={{ fontFamily: '"Playfair Display", Georgia, serif' }}>
                Samay Pizza
              </span>
              <span className="text-xs" style={{ color: '#FFCDD2' }}>Gorakhpur</span>
            </div>
          </Link>
      ...
      {/* Mobile Menu */}
      {mobileOpen && (
        <div className="md:hidden border-t border-red-800 px-4 py-3 space-y-1" style={{ backgroundColor: '#8B0000' }}>
          {navLinks.map(link => (
            <Link
              key={link.path}
              to={link.path}

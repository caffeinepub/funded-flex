...
function validate(data: FormData): FormErrors {
  ...
  else if (!/^[6-9]\d{9}$/.test(data.phone.trim())) errors.phone = 'Enter a valid 10-digit mobile number';
...
export default function OrderPage() {
  ...
  const handlePlaceOrder = async () => {
    ...
    window.open(`https://wa.me/918957401994?text=${encodeURIComponent(message)}`, '_blank');
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4" style={{ backgroundColor: '#FAFAFA' }}>
        <div className="text-center max-w-md">
          ...
          <h2 className="text-2xl font-bold text-gray-800 mb-2" style={{ fontFamily: '"Playfair Display", Georgia, serif' }}>
            Order Sent! 🎉
          </h2>
          <p className="text-gray-500 mb-6">
            Your order has been sent via WhatsApp. We'll confirm it shortly. Please complete the payment via UPI.
          </p>
          <Link
            to="/menu"
        ...
        {items.length === 0 ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
              ...
              <div className="bg-white rounded-2xl shadow-card overflow-hidden border border-gray-100">
                <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                  <h2 className="font-bold text-gray-800 text-lg flex items-center gap-2">
                    <ShoppingCart className="w-5 h-5" style={{ color: '#B71C1C' }} />
                    Cart Summary
                  </h2>
                  <Link to="/menu" className="text-sm font-medium hover:underline" style={{ color: '#B71C1C' }}>
                    + Add More
                  </Link>
                </div>
                  ...
                  {items.map(item => (
                    <div key={item.id} className="px-5 py-3.5 flex items-center gap-3">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-400">{item.size} · {item.category}</p>
                      </div>
                      ...
                      <span className="font-semibold text-sm w-16 text-right" style={{ color: '#B71C1C' }}>
                ...
                <div className="px-5 py-4 border-t flex items-center justify-between" style={{ backgroundColor: '#FFF8F0' }}>
                  <span className="font-bold text-gray-800">Total</span>
                  <span className="font-bold text-xl" style={{ color: '#B71C1C' }}>₹{totalPrice}</span>
                </div>
              </div>
              ...
              <div className="bg-white rounded-2xl shadow-card border border-gray-100 p-5">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    />
                    {errors.name && <p className="text-xs mt-1" style={{ color: '#B71C1C' }}>{errors.name}</p>}
                  </div>
                  ...
                  <div>
                    ...
                    />
                    {errors.phone && <p className="text-xs mt-1" style={{ color: '#B71C1C' }}>{errors.phone}</p>}
                  </div>

                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">
                      Delivery Address <span style={{ color: '#B71C1C' }}>*</span>
                    </label>
                    <textarea
                      name="address"
                      value={form.address}
                    ...
                    />
                    {errors.address && <p className="text-xs mt-1" style={{ color: '#B71C1C' }}>{errors.address}</p>}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="lg:col-span-2 space-y-5">
              {/* Place Order */}
              <div className="bg-white rounded-2xl shadow-card border border-gray-100 p-5">
                <h2 className="font-bold text-gray-800 text-lg mb-4">Place Order</h2>
                <div className="rounded-xl p-4 mb-4 text-sm" style={{ backgroundColor: '#FFF8F0', border: '1px solid #FFCDD2' }}>
                  <p className="text-gray-600">
                    Clicking <strong>"Place Order via WhatsApp"</strong> will open WhatsApp with your complete order details pre-filled.
                  </p>
                </div>
                <button
                ...
                <p className="text-xs text-gray-400 text-center mt-2">
                  Order will be sent to +91 89574 01994
                </p>
              </div>

              {/* UPI Payment */}
              <div className="bg-white rounded-2xl shadow-card border border-gray-100 p-5">
                <h2 className="font-bold text-gray-800 text-lg mb-1">Pay via UPI</h2>
                <p className="text-gray-500 text-sm mb-4">Scan QR code or use UPI ID to pay</p>

                {/* QR Code */}
                <div className="flex justify-center mb-4">
                  <div className="p-3 rounded-xl border-2" style={{ borderColor: '#B71C1C' }}>
                    <img
              ...
              {/* Order Total Summary */}
              <div className="rounded-xl p-4" style={{ backgroundColor: '#B71C1C' }}>
